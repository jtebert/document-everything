...
Git LFS (Large File Storage) is a way of storing large binary files like data, images, or videos. Normally, if you made any change to the file, the whole thing would be re-added to the repository and the size of your repository would blow up over time.

Git LFS avoids this by not directly storing the file in the repository, but instead storing a pointer to the file, and putting the file elsewhere (like somewhere on GitHub). But you don't have to actually understand how that works to use it. You just install it and it's magic.

### Installation (for Ubuntu)

1. If you're on Ubuntu 18.04+, the package is already available through the system repository and you can skip this step. If not, add it with:
   ```shell
...
1. Add LFS to your repository. (You need to do this once per repository, from within the repository directory.)
   ```shell
   git lfs install
   ```
2. Add a file extension to use with git LFS:
   ```shell
   git lfs track "**/*.jpg"
   ```
   You can always add more later.
3. Make sure that `.gitattributes` is added to your repository to keep track of this.
   ```shell
   git add .gitattributes
   ```
4. Commit & push (to Github) files of this type just like you normally would.

### Migration issues

So far, this works if you're keeping track of new files, but any files you had around from before making this change are still taking up space in your repository. There's a way to fix/change that, but I haven't tried it yet...

Sources: [Github LFS](https://git-lfs.github.com/), [git lfs wiki](https://github.com/git-lfs/git-lfs/wiki/Installation), [Neflify large media docs](https://docs.netlify.com/large-media/setup/#migrate-files-from-git-history)
{:.fs-2}